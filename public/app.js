(function () {
  ...
  async function includePartials() {
    ...
    document.querySelectorAll('[data-include="header"]').forEach((slot) => {
      tasks.push(fetch('partials/header.html').then((r) => r.text()).then((html) => { slot.outerHTML = html; }));
    });
    document.querySelectorAll('[data-include="footer"]').forEach((slot) => {
      tasks.push(fetch('partials/footer.html').then((r) => r.text()).then((html) => { slot.outerHTML = html; }));
    });
